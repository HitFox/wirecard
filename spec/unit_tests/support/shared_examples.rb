...
RSpec.shared_examples 'stub requests' do
  before do
    ...
    stub_request(:post, "https://checkout.wirecard.com/seamless/frontend/init").
      with(body: {"amount"=>"1000", "cancelUrl"=>"http://localhost.cancel.url", "confirmUrl"=>"http://localhost.confirm.url", "consumerIpAddress"=>"127.0.0.1", "consumerUserAgent"=>"some agent", "currency"=>"USD", "customerId"=>"D200001", "failureUrl"=>"http://localhost.failure.url", "language"=>"en", "orderDescription"=>"some description", "paymentType"=>"CCARD", "requestFingerprint"=>"0b1708071acbbc903bf4a87e7094d23d5b9c443effaa9936ec970839dc3c18e38010df6ade94b8de0bc7d50caea8c20364b65e9ce22d0dc53b457d184eba62a9", "requestFingerprintOrder"=>"customerId,shopId,language,currency,successUrl,failureUrl,cancelUrl,serviceUrl,confirmUrl,paymentType,amount,consumerIpAddress,consumerUserAgent,orderDescription,requestFingerprintOrder,secret", "serviceUrl"=>"http://localhost.service.url", "shopId"=>"qmore", "successUrl"=>"http://localhost.success.url"},
                  headers: {'Accept'=>'*/*', 'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'Connection'=>'close', 'Content-Length'=>'800', 'Content-Type'=>'application/x-www-form-urlencoded', 'Host'=>'checkout.wirecard.com', 'User-Agent'=>'### User Agent ###'}).
